    ...
    useEffect(() => {
        ...
        getExercisesForPlanDay(plan.id, day)
            .then(data => {
                ...
                const initialData = {};
                data.forEach(ex => {
                    // ZMIANA: Inicjalizacja danych w zależności od typu
                                ...
                                {exercise.exerciseType === 'STRENGTH' ? (
                                    <>
                                        ...
                                        {formData[exercise.exerciseId]?.map((set, index) => (
                                            <div key={index} className="grid grid-cols-12 gap-2 items-center mb-2">
                                                ...
                                                <input type="number" placeholder="kg" className="col-span-4 p-3 bg-surfaceDarkGray text-center" value={set.weight} onChange={(e) => handleSetChange(exercise.exerciseId, index, 'weight', e.target.value)} />
                                                <input type="number" placeholder="reps" className="col-span-4 p-3 bg-surfaceDarkGray text-center" value={set.reps} onChange={(e) => handleSetChange(exercise.exerciseId, index, 'reps', e.target.value)} />
                                    ...
                                    <div className="grid grid-cols-2 gap-4 pt-2">
                                        <div>
                                            ...
                                            <input type="number" placeholder="min" className="w-full p-3 bg-surfaceDarkGray text-center rounded-lg" value={formData[exercise.exerciseId][0].durationMinutes} onChange={(e) => handleSetChange(exercise.exerciseId, 0, 'durationMinutes', e.target.value)} />
                                        </div>
                                         <div>
                                            <label className="text-xs text-borderGrayHover font-semibold mb-1 block">Distance (km)</label>
                                            <input type="number" step="0.1" placeholder="km" className="w-full p-3 bg-surfaceDarkGray text-center rounded-lg" value={formData[exercise.exerciseId][0].distanceKm} onChange={(e) => handleSetChange(exercise.exerciseId, 0, 'distanceKm', e.target.value)} />
                                        </div>
                                    </div>
                                )}
                            </div>
